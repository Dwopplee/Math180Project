...
# Makes oscillatory data
# Takes input that determines form of noise
make.osc.data = function(noise = rnorm(1000)) {
  data = as.data.frame(replicate(10, rnorm(1000)))
  names(data) = c("x1", "x2", "x3", "x4", "x5", "x6", "x7", "x8", "x9", "x10")
  
  coeffs = runif(10, -2, 2)
  
  truth = coeffs[1] * sin(coeffs[2] * data$x1) + coeffs[3] * cos(coeffs[4] * data$x2) + 
    coeffs[5] * cos(coeffs[6] * data$x3) + coeffs[7] * data$x7 + coeffs[8] * data$x8 + 
    coeffs[9] * data$x9 + coeffs[10] * data$x10
  y = truth + noise
...
run.osc.test = function(noise = rnorm(1000)) {
  data = make.osc.data()
...
run.osc.trials = function(trials, noise = rnorm(1000), main) {
  errors = replicate(trials, run.osc.test(noise = noise))
...
norm.errors = run.osc.trials(20, main = "Gaussian Noise")
laplace.errors = run.osc.trials(20, noise = r(laplace)(1000), main = "Laplace Noise")
cauchy.errors = run.osc.trials(20, noise = rcauchy(1000), main = "Cauchy Noise")